...
$sessionName = "wmux_phase7b_$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
$marker = "WMUX_PHASE7B_$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
...
if ($sessionCount -ne 0) {
  throw "window-switching test requires an empty daemon because it restarts wmux with WMUX_DEFAULT_SHELL=cmd.exe /D /Q"
}
...
try {
  ...
  Write-Host "window 0: write marker"
  ...
  Write-Host "Ctrl+b c: create window"
  ...
  Write-AttachInput -Pipe $attach -Text "echo $marker`_W1`r"
  [void](Read-UntilMarker -Pipe $attach -Pattern ([regex]::Escape("$marker`_W1")) -Description "window 1 marker")

  Write-Host "Ctrl+b p: previous window"
  Write-AttachCommand -Pipe $attach -Command "previous-window"
  [void](Read-UntilMarker -Pipe $attach -Pattern ([regex]::Escape("$marker`_W0")) -Description "window 0 replay")
  Write-AttachInput -Pipe $attach -Text "echo $marker`_W0B`r"
  [void](Read-UntilMarker -Pipe $attach -Pattern ([regex]::Escape("$marker`_W0B")) -Description "window 0 second marker")

  Write-Host "Ctrl+b n: next window"
  Write-AttachCommand -Pipe $attach -Command "next-window"
  [void](Read-UntilMarker -Pipe $attach -Pattern ([regex]::Escape("$marker`_W1")) -Description "window 1 replay")
  Write-AttachInput -Pipe $attach -Text "echo $marker`_W1B`r"
  [void](Read-UntilMarker -Pipe $attach -Pattern ([regex]::Escape("$marker`_W1B")) -Description "window 1 second marker")
  ...
  if (($windows.Output -split "`r?`n").Count -ne 2) {
    throw "expected exactly two windows after interactive create`n$($windows.Output)"
  }

  Write-Host "ok: interactive new/previous/next window switching preserved independent shell state"
}
finally {
  if ($attach -ne $null) {
    $attach.Dispose()
