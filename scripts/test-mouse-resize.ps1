...
function New-AttachFrame {
  param(
    [ValidateSet("Input", "Detach", "Command", "MouseEvent")]
    [string]$Type,
...
function Write-AttachMouseEvent {
  param(
    ...
    [ValidateSet("Press", "Release", "Drag")]
    [string]$Action = "Press"
  )

  $buttonCode = switch ($Action) {
    "Press" { 0 }
    "Release" { 0 }
    "Drag" { 32 }
  }
  ...
  $payload[6] = [byte]$button
  $payload[7] = [byte]$actionCode
...
$sessionName = "wmux_phase11c_$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
$marker = "WMUX_PHASE11C_$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
...
if ($sessionCount -ne 0) {
  throw "mouse-resize test requires an empty daemon because it restarts wmux with WMUX_DEFAULT_SHELL=cmd.exe /D /Q"
}
...
try {
  ...
  Write-AttachInput -Pipe $attach -Text "set WMUX_RESIZE=P1`r"
  ...
  Write-AttachInput -Pipe $attach -Text "set WMUX_RESIZE=P2`r"
  [void](Read-UntilMarker -Pipe $attach -Pattern ">" -Description "pane 2 prompt after identity")

  Write-Host "mouse click: focus pane 1"
  Write-AttachMouseEvent -Pipe $attach -Column 2 -Row 2 -Action Press
  Write-AttachInput -Pipe $attach -Text "echo %WMUX_RESIZE%_$marker`_LEFT`r"
  [void](Read-UntilMarker -Pipe $attach -Pattern ([regex]::Escape("P1_$marker`_LEFT")) -Description "pane 1 click-routed input")

  Write-Host "mouse drag: shrink pane 1"
  Write-AttachMouseEvent -Pipe $attach -Column 45 -Row 2 -Action Press
  Write-AttachMouseEvent -Pipe $attach -Column 30 -Row 2 -Action Drag
  Write-AttachMouseEvent -Pipe $attach -Column 30 -Row 2 -Action Release

  Write-Host "mouse click: column 35 should now focus pane 2"
  Write-AttachMouseEvent -Pipe $attach -Column 35 -Row 2 -Action Press
  Write-AttachInput -Pipe $attach -Text "echo %WMUX_RESIZE%_$marker`_RIGHT`r"
  [void](Read-UntilMarker -Pipe $attach -Pattern ([regex]::Escape("P2_$marker`_RIGHT")) -Description "pane 2 after drag-resize")

  Write-AttachDetach -Pipe $attach
  $attach.Dispose()
  $attach = $null

  Write-Host "ok: mouse drag resized split and input routed using updated pane geometry"
}
finally {
  if ($attach -ne $null) {
    $attach.Dispose()
