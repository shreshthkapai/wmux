...
fn apply_sgr(screen: &mut Screen, params: &[u16]) {
    ...
    while index < params.len() {
        match params[index] {
            ...
            90..=97 => screen.set_fg(Some(Color::Indexed((params[index] - 90 + 8) as u8))),
            100..=107 => screen.set_bg(Some(Color::Indexed((params[index] - 100 + 8) as u8))),
            38 | 48 => {
...
mod tests {
    ...
    fn sgr_styles_are_stored_per_cell() {
        ...
        assert_eq!(x_style.fg, Some(Color::Indexed(1)));
        assert_eq!(x_style.bg, Some(Color::Indexed(24)));
    ...
    fn replays_cmd_startup_without_prompt_gap() {
        ...
        engine.feed(&mut screen, b"C:\\Users\\shres\\mux\\wmux\\wmux-clean>");

        assert_eq!(
            screen.grid().line(0).unwrap().text(),
            "C:\\Users\\shres\\mux\\wmux\\wmux-clean>"
        );
        assert_eq!(screen.grid().line(1).unwrap().text(), "");
        assert_eq!(screen.cursor(), (0, 35));
    }
